...
SELECT COUNT(id) FROM employees3; --7 ama NULL olsaydı sayılmazdı
...
SELECT calisan_isim isim FROM workers; --AS olmadan da çalışır

SELECT calisan_isim isim FROM workers AS w; --tablo ismi w oldu
SELECT calisan_isim isim FROM workers w; --AS olmadan da çalışır
...
-- SELECT * FROM employees3 WHERE maas = 7000; --7000i elle yazdık, çalışır ama olmaz
...
SELECT MAX (maas) FROM employees3 WHERE maas < (7000); 

SELECT MAX (maas) FROM employees3 WHERE maas < (SELECT MAX (maas) FROM employees3); --parantez içine 7000 yerine MAX'i bulan kodu yazdık
...
SELECT marka_id,calisan_sayisi FROM brands WHERE marka_isim IN ('Pierre Cardin','Adidas','Vakko');
...
--SELECT isim,maas,isyeri FROM employees3 WHERE isyeri IN (SELECT marka_isim FROM brands WHERE calisan_sayisi <15000); BURAYI İNCELE YANLŞLIK VARCHAR
...
WHERE EXISTS (SELECT urun_id,musteri_isim FROM april AS a WHERE a.urun_isim = m.urun_isim);
...
FROM april AS a
WHERE EXISTS (SELECT * FROM march AS m WHERE a.urun_isim=m.urun_isim);
