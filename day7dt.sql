...
SELECT * FROM person WHERE soyisim = 'Bulut' ORDER by isim DESC;
--alternatif
SELECT * FROM person WHERE soyisim = 'Bulut' ORDER by 2 DESC; --sütunun sırasını yazdık 
...
SELECT * FROM words WHERE word ~ 'h[ai]t'; --köşeli parantez içi veya demek 

--CASE-INSENSITIVE
SELECT * FROM words WHERE word ~* 'h[ai]t'; --büyük küçük harf duyarsız oldu
...
SELECT * FROM words WHERE word ~* 'h[a-k]t'; -- (- işareti harflerin arasında anlamında olur)
...
SELECT * FROM words WHERE word ~* 'm$';
...
SELECT * FROM words WHERE word ~* '^y(.*)f$'; --parantez olmasa da olur
...
SELECT SUBSTRING ('sqlders',1,3);--"sql" --Java’da substring index mantigiyla 0'dan baslarken, burada 1'den baslar.
...
SELECT SUBSTRING (sehir,2,4)--2'den basla 4 karakter al, --stan
...
SELECT * FROM developers ORDER BY id FETCH NEXT 3 ROWS ONLY;
...
INSERT INTO employees5 (id,sehir) VALUES (123,'Ankara'); --not null CONSTRAINT den dolayı hata verdi 
...
ADD /*CONSTRAINT companies2_pk */ PRIMARY KEY (sirket_id); --not null, unique
...
--Kısıtlamaları görmek için sorgu
