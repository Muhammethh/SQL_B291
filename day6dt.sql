...
UNION 
SELECT sehir, maas FROM employees4 WHERE isyeri = 'Vakko';  -- Burda city,sehir ve sehir,maaş data tiplerinden dolayı aynı sıralarda olmalı 
...
SELECT city FROM developers INTERSECT SELECT sehir FROM employees4;
...
FROM developers 
WHERE prog_lang = 'Java' 
INTERSECT SELECT sehir FROM employees4 WHERE maas > 1000;

--Hangi tablo sorgusunun önce yazılacağı önemli değildir
...
SELECT urun_isim FROM march UNION SELECT urun_isim FROM april;
SELECT urun_isim FROM march INTERSECT SELECT urun_isim FROM april;
SELECT urun_isim FROM march EXCEPT SELECT urun_isim FROM april;
-----------------------------------------
 --33- JOIN
...
SELECT companies2.sirket_id, sirket_isim,siparis_tarihi,siparis_id FROM companies2 INNER JOIN orders ON companies2.sirket_id = orders.sirket_id;
 --ON ile soft relationship yaptık. Tablo sırası inner joinde önemli değil 
...
SELECT companies2.sirket_id, sirket_isim,siparis_tarihi,siparis_id FROM companies2 LEFT JOIN orders ON companies2.sirket_id = orders.sirket_id;
...
list order date, order id.*/
SELECT orders.sirket_id, sirket_isim,siparis_tarihi,siparis_id FROM companies2 RIGHT JOIN orders ON companies2.sirket_id = orders.sirket_id;

--NOT:LEFT/RIGHT JOIN tablo sırası değiştirilerek birbirinin yerine alternatif olarak kullanılabilir.

SELECT orders.sirket_id, sirket_isim,siparis_tarihi,siparis_id FROM orders LEFT JOIN companies2 ON companies2.sirket_id = orders.sirket_id;
...
FROM staff p INNER JOIN staff m ON p.yonetici_id = m.id;
...
SELECT * FROM developers WHERE name LIKE 'A%'; --A, Ab, Abc ile başlayanlar
...
--Write QUERY that prints all the information of developers whose prog. language has 4 letters and the third letter is 
...
--1. '[cb]at' ==> 'cat' veya 'bat' ile eşleşir. Bir kelimenin icinde cat olsa o da eslesir. (ascatdat) da true verir
...
SELECT 'z' ~ '[^a-e]' as result; --true, çünkü z bu aralığın dışında
