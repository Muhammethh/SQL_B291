...
INSERT INTO address VALUES('10003','Kaya Sok', '50.Cad.', 'Ankara');  --başarılı, PK dublicate olabilir
INSERT INTO address VALUES('10002','Taş Sok', '30.Cad.', 'Konya'); --başarılı
INSERT INTO address VALUES('10012','Taş Sok', '30.Cad.', 'Konya'); --hata 10012 değeri yukarda(PK) yok 
...
DELETE FROM students1 WHERE isim = 'Ahmet Ran' OR isim = 'Veli Han'; -- AYNI FİELDDAN İŞLEM YAPILACAĞI ZAMAN OR FARKLI FİELDLERDE AND KULLANILIYOR
...
INSERT INTO doctors(name,salary,email) VALUES('',6000,'doc@mail.com'); --Başarılı, hiçlik null ile aynı değildir


--doctors tablosundan tüm kayıtları silelim.
--Delete all records from the doctors table.

TRUNCATE TABLE doctors;  --where ile kullanılmaz 
...
DELETE FROM worker; --silemeyiz çünkü tablonun childı var, 

--worker tablosunda address tarafından referans alınan kayıtlar olduğu 
--için silmeye izin yok

DELETE FROM worker WHERE id = '10002';--HATA, çünkü childda referans var 


--id=10002 olan kayıt adresler tarafından referans alındığı 
--için silmeye izin yok.Once child'dan iliskiyi koparalim

DELETE FROM address WHERE adres_id = '10002'; --şimdi artık parentdan yani workerdan da id'si 10002 olanı silebiliriz

DELETE FROM worker WHERE id = '10002'; --silindi, çünkü referansı silmiştik

DELETE FROM address; --tüm ilişki koparıldı çünkü tablo tamamen silindi
...
---
DELETE FROM grades2 WHERE talebe_id = 123; --başarılı, childdan bir kaydı doğrudan silebiliriz

DELETE FROM students2 WHERE id = 126; --ON DELETE CASCADE den dolayı önce childden sonra parenttan sildi

DELETE FROM students2; --referans alınmayan tarih kalmıştı

DELETE FROM grades2; -- ile nul olan, referans alınmayan da silindi

--------------------------
--19-tabloyu silme

DROP TABLE students2; --HATA, tabloyu silemezsiniz çünkü childı var

DROP TABLE students2 CASCADE; --childda ki FK kopar, child tablo silinmez

DROP TABLE if exists students18; --varsa bu tabloyu sil, yoksa hata verme(IF EXISTS)
