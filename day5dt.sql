...
SELECT * 
FROM person 
ORDER BY adres ASC; --ASC zaten doğal sıralamadır yazmasak da olurdu


SELECT * 
FROM person 
ORDER BY adres, isim, soyisim; --sütunda aynı olanlar arasında bi sonra ki sütün içinde sıralama yapıyor
...
FROM person
ORDER BY LENGTH (soyisim) --LENGTH sütunu açarak isim verdik, karakter uzunlupunu görmek için
---
SELECT isim,soyisim, LENGTH (soyisim) AS karakter_sayisi
FROM person
ORDER BY karakter_sayisi -- ORDER BY sorgusunu yeni isimle yaptık
...
SELECT AVG (maas) FROM employees4;--ortalama maaşı bulduk

SELECT isim,sehir,maas FROM employees4
...
FROM greengrocer
GROUP BY isim
ORDER BY toplam_kg DESC; -- yada ORDER BY SUM (urun_miktar) DESC;
...
SELECT isim, SUM(urun_miktar) AS toplam_kg, COUNT (DISTINCT urun_adi) AS cesit_sayisi
FROM greengrocer
GROUP BY isim;
...
--WHERE kayıtları filtreler, HAVING belli gruplarda ki sonuçları filtreler
...
SELECT sehir, COUNT(id) AS kisi_sayisi
FROM personel
GROUP BY sehir
HAVING COUNT(id)>1;
